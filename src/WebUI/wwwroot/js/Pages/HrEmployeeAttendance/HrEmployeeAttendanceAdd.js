$(document).ready(function () {
    ...
    function loadAllAttendance(code) {
        ...
        var gridattend = $("#attendancegrid").kendoGrid({
            dataSource: {
                transport: {
                    read: "/HrEmployeeAttendance/GetEmployeeAttendancByCode?code=" + code + "&&date=" + $("#attendDate").val()
                }, schema: {
                    model: {
                        id: "id",
                        fields: {
                            ...
                            dayName: { type: "string" },
                            attendType: { defaultValue: { attendid: "", attendType: "" } },
                            time: { type: "string" },
            ...
            columns: [
                ...
                },
                {
                    ...
                    width: Resources.CodeWidth
                    //    template: '#if(  attendType.indexOf("In")>0) {# #:Resources.Attend # #} else if(attendType=="0") {# <select class="selectAddAttendType"><option value="2">اجازه</option ><option value="1">غياب</option><option value="3">عطلة اسبوعية</option></select> #} else {# #:Resources.Leave # #}#',
                }
            ...
            dataBound: function (e) {
                e.sender.items().each(function () {
                    ...
                    $(ddt).kendoDropDownList({
                        value: dataItem.attendType,
                        dataSource: ddlDataSource,
                        dataTextField: "attendType",
                        dataValueField: "attendid",
                        change: onDDLChange,
                        //index:1
                    });
                });
    ...
    $("#attendDate").change(function () {
        loadAllAttendance($("#empCode").val());
    })
})

function columnTemplateFunction(dataItem) {
    if (dataItem.attendType.indexOf("In") > 0) {
        var input = '<input value="' + Resources.Attend + '" readonly/>'
        return input;
    } else if (dataItem.attendType.indexOf("Out") > 0) {
        var input = '<input value="' + Resources.Leave + '" readonly/>'
        return input;
    } else {
...
function onDDLChange(e) {
    ...
    dataItem.set("attendType", e.sender.value());
    dataItem.set("attendid", e.sender.value());
};
