$(document).ready(function () {
    ...
    $('input[name="IsPosted"]').change(function () {
        ...
        if (!hasRoleEdit == true) {
            ...
            for (var i = 0; i < gridData.length; i++) {
                $(".btnCheckInvoice").removeAttr('disabled');
            }
        }
    });
    ...
    $("#NoticeType").kendoDropDownList({
        ...
        dataSource: {
            ...
            transport: {
                read: {
                    url: "/RceNoticeBond/GetAllNoticeTypeForDDL",
                },
    ...
    function onSelectRceClient(e) {
        loadRceNoticeBondDetailsGrid(e.dataItem.id);
        var pervTotalAmount = parseFloat($("#prevTotalAmount").val());
        var prevTotalTaxAmount = parseFloat($("#prevTotalTaxAmount").val());
        var prevFK_RceClientId = parseFloat($("#prevFK_RceClientId").val());

        if (e.dataItem.id == prevFK_RceClientId) {
            $('#TotalAmount').val(pervTotalAmount);
            $('#TotalTaxAmount').val(prevTotalTaxAmount);
            setTxtGross();
    ...
    loadRceNoticeBondDetailsGrid(parseInt($("#FK_RceClientId").val()));

    function loadRceNoticeBondDetailsGrid(clientId) {
        ...
        var dataSource = new kendo.data.DataSource({
            ...
            transport: {
                read: {
                    url: "/RceNoticeBond/GetAllRceInvoices?clientId=" + clientId + "&dateFrom=" + $("#DateFrom").val() + "&dateTo=" + $("#DateTo").val() + "&noticeId=" + parseInt($("#Id").val()) + "&branchId=" + parseInt($("#FK_DefBranchId").val()),
                    Type: "GET"
                }
            },
            error: function (e) {
        ...
        var grid = $("#GridNoticeDetail").kendoGrid({
            ...
            dataBound: function (e) {
                ...
                if ($('input[name="IsPosted"]').prop("checked") == true) {
                    $(".btnCheckInvoice").attr('disabled', 'disabled');
                }
            }
        });
...
function checkPeriodAndSaveNoticeBondEdit() {
...
function SubmitRceNoticeBondEdit() {
                ...
                success: function (result) {
                    ...
                    if (result) {
                        ...
                        swal({
                            ...
                            window.location.href = '/RceNoticeBond/Index';
                        });
                    }
                    else {
                        swal({
