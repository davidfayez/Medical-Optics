$(document).ready(function () {
    ...
    LoadGridRceRepaymentBonds();
    function LoadGridRceRepaymentBonds() {
        var grid = $("#rceRepaymentBondsGrid").kendoGrid({
            excel: {
                fileName: "Rce Repayment Bonds.xlsx",
                allPages: true,
                filterable: true
            },
            dataSource: {
                transport: {
                    read: "/RceRepaymentBond/GetAll"
                },
                schema: {
                    model: {
                        id: "id",
            ...
            { width: Resources.DoubleActionWidth, template: "<a  href='/RceRepaymentBond/Edit/#= id #'  class='btn btn-success btn-sm btnEdit'><i class='fas fa-edit'></i></a> <a  class='btn btn-danger btn-sm btnDelete' ><i class='fas fa-trash-alt'></i></a> <a  class='btn btn-warning btn-sm btnOpenVoucherReport'><i class='fas fa-eye'></i></a>" },
        ...
        grid.data("kendoGrid").table.on("click", ".btnDelete", removeRceRepaymentBond);
        grid.data("kendoGrid").table.on("click", ".btnOpenVoucherReport", OpenVoucherReport);
    }

    function removeRceRepaymentBond() {

        var row = $(this).closest("tr"),
            grid = $("#rceRepaymentBondsGrid").data("kendoGrid"),
        ...
        swal({
            ...
            setTimeout(function () {
                $.ajax({
                    url: "/RceRepaymentBond/Delete?id=" + dataItem.id,
                    type: "Get",
    ...
    $("#DefBranches").change(function () {
        ...
        $('#rceRepaymentBondsGrid').data('kendoGrid').dataSource.read({ fK_DefBranchId: fK_DefBranchId });
    });
});

$(".exportExcel").on('click', function () {
    $("#rceRepaymentBondsGrid").getKendoGrid().saveAsExcel();
});
