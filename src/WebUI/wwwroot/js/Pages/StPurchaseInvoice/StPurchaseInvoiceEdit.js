$(document).ready(function () {
    ...
    tempSource = new kendo.data.DataSource({
        transport: {
            read: {
                url: "/StPurchaseInvoice/GetDetailsById?id=" + $("#Id").val(),
                Type: "GET"
            },
    ...
    $("#btnAddNewDetail").on('click', function () {
        ...
        if ($("#formEditPurInvoice").valid() && FK_StItemId > 0 && QuantityTo > 0 && UnitPurchasePrice > 0 && OperatingNumber != "") {
    ...
    $("#btnSave").click(function () {
        ...
        if ($("#formEditPurInvoice").valid() && $("#SerialNumber").val() > 0 && table.length > 0) {
            var tranDetail = [];
            var stStoreId;
    ...
    $(".btnPrint").on('click', function () {
        var url = "/StPurchaseInvoice/Print?id=" + $("#Id").val();
        window.open(url, '_blank');
    });
    $(".btnExport").on('click', function () {
        $("#purInvoiceDetailGrid").getKendoGrid().saveAsExcel();
    });
}) 