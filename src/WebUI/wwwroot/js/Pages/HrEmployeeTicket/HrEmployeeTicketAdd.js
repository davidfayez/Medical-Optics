$(document).ready(function () {
    ...
    var gridDataSource = new kendo.data.DataSource({
        transport: {
            read: "/HrEmployeeTicket/GetEmployeeKinship?id=" + $("#FK_HrEmployeeId").val()
        }, schema: {
            model: {
                id: "id",
                fields: {
    ...
    $("#saveTicketbtn").bind("click", function () {
        ...
        if (imgFile) {
            ...
            $.ajax({
                url: "/HrEmployeeTicket/UpdateImage",
                type: "POST",
                processData: false,
                contentType: false,
                data: frm,
    ...
    function getCheckedData() {
        ...
        for (var i = 0; i < gridData.length; i++) {
            if (checkedIds[gridData[i].id]) {
                var row = {
                    Id: gridData[i].id,
                    Name: gridData[i].fullNameAr,
                    DateOfBirth: gridData[i].birthDate,
