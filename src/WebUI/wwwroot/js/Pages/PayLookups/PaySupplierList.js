$(document).ready(function () {
    $('#DefBranches').change(function () {
        ...
        $("#PaySupplierAccountsDDLTree").data("kendoDropDownTree").value("");
        $("#PaySupplierAccountsDDLTree").data("kendoDropDownTree").dataSource.read();
    ...
    $("#PaySupplierAccountsDDLTree").kendoDropDownTree({
    ...
    var dataSource = new kendo.data.TreeListDataSource({
        transport: {
            read: {
                url: "/PayLookups/GetAllMainAccountsWithDescendants",
                dataType: "json",
                //dataType: "jsonp"
                Type: "GET"
            },
            parameterMap: function (data, action) {
                ...
                if (action === "read") {
                    return {
                        ...
                        ids: $("#PaySupplierAccountsDDLTree").data("kendoDropDownTree").value().join(", "),
                        date: $("#Date").val(),
                        isActive: isActive,
    ...
    var grid = $("#PaySupplierGrid").kendoTreeList({
        ...
        //toolbar: ["excel"],
        excel: {
            fileName: "Suppliers Accounts.xlsx",
        ...
        columns: [
           ...
           /* { field: "accountNameAr", title: Resources.NameArResource, width: Resources.NameWidth },*/
            ...
            { width: Resources.DoubleActionWidth, template: "#if(isMainAccount != true){ #<a  href='/PayLookups/EditPaySupplier/#= id #'  class='btn btn-success btn-sm btnEdit'><i class='fas fa-edit'></i></a> <a  class='btn btn-danger btn-sm btnDelete' ><i class='fas fa-trash-alt'></i></a> #}#" }
        ],
    ...
    $('#PaySupplierGrid').data('kendoTreeList').dataSource.page(1); // select current page
    grid.data("kendoTreeList").table.on("click", ".btnDelete", removePaySupplier);

    function removePaySupplier() {

        //var deletee = stringify(resources.delete).serialize();

        var row = $(this).closest("tr"),
            grid = $("#PaySupplierGrid").data("kendoGrid"),
        ...
        swal({
            ...
            setTimeout(function () {
                $.ajax({
                    url: "/PayLookups/DeletePaySupplier?Id=" + dataItem.id,
                    type: "Get",
    ...
    $("#btnDataReview").on('click', function () {
        ...
        var accountIds = $("#PaySupplierAccountsDDLTree").data("kendoDropDownTree").value().join(", ");
        var date = $("#Date").val();
    ...
    function RefreshGrid() {
        ...
        $('#PaySupplierGrid').data('kendoTreeList').dataSource.read();
    }
    ...
    //    var grid = $("#PaySupplierGrid").kendoGrid({
    //        excel: {
    ...
    //                }
    //            },
    //            { width: Resources.DoubleActionWidth, template: "<a  href='/PayLookups/EditPaySupplier/#= id #'  class='btn btn-success btn-sm btnEdit'><i class='fas fa-edit'></i></a> <a  class='btn btn-danger btn-sm btnDelete ' ><i class='fas fa-trash-alt'></i></a>" },
    ...
    //        //resizable: true
    //    });
    //    grid.data("kendoGrid").table.on("click", ".btnDelete", removePaySupplier);
    //}

    //$("#DefBranches").change(function () {
    //    var fK_DefBranchId = parseInt($("#FK_DefBranchId").val());
    //    $('#PaySupplierGrid').data('kendoGrid').dataSource.read({ fK_DefBranchId: fK_DefBranchId });
    //});
});
$(".exportExcel").on('click', function () {
    $("#PaySupplierGrid").getKendoGrid().saveAsExcel();
});