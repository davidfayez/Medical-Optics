$(document).ready(function () {
    ...
    loadDeliveryConditionGrid();

    function loadDeliveryConditionGrid() {
        var dataSource = new kendo.data.DataSource({
            transport: {
                read: {
                    url: "/PayLookups/GetAllPayDeliveryCondition",
                    Type: "GET"
                }
            },
            error: function (e) {
        ...
        var grid = $("#PayDeliveryConditionGrid").kendoGrid({
            excel: {
                fileName: "Delivery Condition.xlsx",
            ...
            pageable: {
                ...
                },
                {
                    ...
                    template: '#if(standalone==="True") {#<a  href="/PayLookups/EditPayDeliveryCondition/#= id #?standalone=1"  class="btn btn-success btn-sm btnEdit"><i class="fas fa-edit"></i></a> <a  class="btn btn-danger btn-sm btnDelete" ><i class="fas fa-trash-alt"></i></a>#} else {# <a  href="/PayLookups/EditPayDeliveryCondition/#= id # "  class="btn btn-success btn-sm btnEdit"><i class="fas fa-edit"></i></a> <a  class="btn btn-danger btn-sm btnDelete" ><i class="fas fa-trash-alt"></i></a> #}#'
                },
        ...
        grid.data("kendoGrid").table.on("click", ".btnDelete", removeDeliveryCondition);
    }

    function removeDeliveryCondition() {

        //var deletee = stringify(resources.delete).serialize();

        var row = $(this).closest("tr"),
            grid = $("#PayDeliveryConditionGrid").data("kendoGrid"),
        ...
        swal({
            ...
            setTimeout(function () {
                $.ajax({
                    url: "/PayLookups/DeleteDeliveryCondition?Id=" + dataItem.id,
                    type: "Get",
    ...
    $("#DefBranches").change(function () {
        ...
        $('#PayDeliveryConditionGrid').data('kendoGrid').dataSource.read({ fK_DefBranchId: fK_DefBranchId });
    });
});
$(".exportExcel").on('click', function () {
    $("#PayDeliveryConditionGrid").getKendoGrid().saveAsExcel();
});