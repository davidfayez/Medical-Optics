$(document).ready(function () {
    ...
    var datasource = new kendo.data.DataSource({
        transport: {
            read: "/RceRecurringBond/GetAllRceBond"
        },
        schema: {
            model: {
                id: "id",
    ...
    var Grid = $('#RceBondList').data('kendoGrid');
    ...
    function LoadGridRceBond() {
        var grid = $("#RceBondList").kendoGrid({
            ...
            { width: "150px", template: "<a  href='/RceBond/Edit/#= id #'  class='btn btn-success btn-sm btnEdit'><i class='fas fa-edit'></i></a> <a  class='btn btn-danger btn-sm btnDelete' ><i class='fas fa-trash-alt'></i></a>" },
    ...
    function removeAccount() {
        ...
        swal({
            ...
            setTimeout(function () {
                var dataSource = $("#RceBondList").data("kendoGrid").dataSource;
    ...
    $("#btnDataReview").on('click', function () {
        ...
        var fK_GlFinancialPeriodId = $("#FK_GlFinancialPeriodId").val(),
            rceBondType = $("#RceBondType").val(),
            fK_RceBondId = $("#RceBondSerial").val();

        $('#RceBondList').data('kendoGrid').dataSource.read({ rceBondType: rceBondType, fK_GlFinancialPeriodId: fK_GlFinancialPeriodId, fK_RceBondId: fK_RceBondId });
        datasource.sync();
        var grid = $('#RceBondList').data('kendoGrid');
        var data = grid.dataSource.data();
...
function saveRecurring() {
    ...
    var gridData = $('#RceBondList').data("kendoGrid").dataSource.data();
    for (var i = 0; i < gridData.length; i++) {
        var data = {
            FK_RceBondId: parseInt(gridData[i].id),
            //RceBondType: parseInt($("#RceBondType").val()),
    ...
    if (List.length == 0) {
        ...
        $.ajax({
            url: "/RceRecurringBond/saveRecurring",
            type: "Post",
            ...
            success: function (result) {
                ...
                if (result) {
                    ...
                    swal({
                        ...
                        window.location.href = '/RceRecurringBond/Index';
                    });
                }
                else {
                    swal({
