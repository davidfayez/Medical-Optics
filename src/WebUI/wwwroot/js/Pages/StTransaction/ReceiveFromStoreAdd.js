$(document).ready(function () {
    ...
    tempSource = new kendo.data.DataSource({
        transport: {
            read: {
                url: "/StTransaction/GetStReceiveFromStoreDetailsById?id=" + id,
                Type: "GET"
            },
    ...
    var ReceiveFromStore = $("#ReceiveFromStoreDetailGrid").kendoGrid({
    ...
    $("#ReceiveFromStoreDetailGrid").data("kendoGrid").table.on("click", ".row-checkbox", selectRow);
    ...
    $("#btnApprove").bind("click", function () {
        ...
        var prefix = document.getElementById("SerialPrefixPurchaser").innerText;
        var SerialPrefix = prefix + $("#SerialNumberPurchaser").val();
        ...
        swal({
            ...
            setTimeout(function () {
                ...
                var tran = {
                    Id: id,
                    //SerialNumber: $("#SerialNumberPurchaser").val(),
                ...
                $.ajax({
                    url: '/StTransaction/ApproveReceiveFromStore',
                    type: 'POST',
                    data: { ReceiveFromStoreVM: tran },
                        ...
                        if (result) {
                            ...
                            swal({
                                ...
                                type: "success"
                                    ...
                                    window.location.href = '/StTransaction/EditReceiveFromStore/' + url.substring(url.lastIndexOf("/") + 1, url.length);
                            });
                        }
                        else {
                            swal({
    ...
    $("#btnCancel").bind("click", function () {
        ...
        swal({
            ...
            setTimeout(function () {
                ...
                var tran = {
                    Id: id,
                    SerialNumber: $("#SerialNumberForPurchaser").val(),
                    FK_StStoreFromId: $("#FK_StStoreFromId").val(),
                    FK_StStoreToId: $("#FK_StStoreToId").val(),
                    TransactionDate: $("#TransactionDate").val(),
                    DeliveryNumber: $("#DeliveryNumber").val(),
                ...
                $.ajax({
                    url: '/StTransaction/CancelReceiveFromStore',
                    type: 'POST',
                    data: { ReceiveFromStoreVM: tran },
                    success: function (result) {
                        ...
                        if (result) {
                            ...
                            swal({
                                ...
                                window.location.href = '/StTransaction/IndexReceiveFromStore';
                            });
                        }
                        else {
                            swal({
    ...
    function getCheckedData() {
        ...
        gridData = $("#ReceiveFromStoreDetailGrid").data("kendoGrid").dataSource._data;
        for (var i = 0; i < gridData.length; i++) {
            ...
            if (checkedIds[gridData[i].id]) {
                var row = {
                    ...
                    TotalSalsePrice: gridData[i].unitSalesPrice * gridData[i].quantityFrom,
...
function selectRow() {
    var checked = this.checked,
        row = $(this).closest("tr"),
        grid = $("#ReceiveFromStoreDetailGrid").data("kendoGrid"),
