$(document).ready(function () {
    ...
    $("#btnAddNewDetail").on('click', function () {
        ...
        if ($("#mainForm").valid() && FK_StItemId > 0 && QuantityFrom > 0 && UnitSalesPrice > 0) {
            ...
            $.ajax({
                url: "/StTransaction/CheckItemBalance?itemId=" + FK_StItemId + "&&stockId=" + stStoreId + "&&quantity=" + QuantityFrom,
    ...
    $("#btnSaveBalance").click(function () {
        ...
        if ($("#mainForm").valid() && $("#SerialNumber").val() > 0 && $("#DeliveryNumber").val() > 0 && table.length > 0) {
            ...
            $.ajax({
                url: '/StTransaction/CreateBranchExchange',
                type: 'POST',
                ...
                success: function (result) {
                    if (isNaN(result)) {
                        ...
                        swal({
                            ...
                            window.location.href = '/StTransaction/EditBranchExchange?id=' + result;
                        });
                    } else {
                        swal({
