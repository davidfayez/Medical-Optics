...
$(document).ready(function () {
    ...
    function loadContractTypeGrid() {
        var dataSource = new kendo.data.DataSource({
            transport: {
                read: {
                    url: "/HrLookups/GetAllContractType",
                    Type: "GET"
                }
            },
            error: function (e) {
        ...
        var grid = $("#GridContractType").kendoGrid({
            excel: {
                fileName: "Contract Types.xlsx",
                allPages: true,
                filterable: true
            },
            ...
            columns: [
                ...
                { width: Resources.DoubleActionWidth, template: "<a  href='/HrLookups/EditContractType/#= id #'  class='btn btn-success btn-sm btnEdit'><i class='fas fa-edit'></i></a> <a  class='btn btn-danger btn-sm btnDelete' ><i class='fas fa-trash-alt'></i></a>" },
        ...
        grid.data("kendoGrid").table.on("click", ".btnDelete", removeContractType);
    }

    function removeContractType() {

        var row = $(this).closest("tr"),
            grid = $("#GridContractType").data("kendoGrid"),
        ...
        swal({
            ...
            setTimeout(function () {
                $.ajax({
                    url: "/HrLookups/DeleteContractType?id=" + dataItem.id,
                    type: "Get",
    ...
    $("#DefBranches").change(function () {
        ...
        $('#GridContractType').data('kendoGrid').dataSource.read({ fK_DefBranchId: fK_DefBranchId });
    });
});

$(".exportExcel").on('click', function () {
    $("#GridContractType").getKendoGrid().saveAsExcel();
});
...
function removeContractTypeEdit(id) {
    ...
    swal({
        ...
        setTimeout(function () {
            $.ajax({
                url: "/HrLookups/DeleteContractType?id=" + id,
                type: "Get",
                contentType: 'application/json; charset=utf-8',
                success: function (result) {
                    ...
                    if (result) {
                        swal({
                            ...
                            window.location.href = '/HrLookups/IndexContractType'
                        });
                    }
                    else {
                        swal({
