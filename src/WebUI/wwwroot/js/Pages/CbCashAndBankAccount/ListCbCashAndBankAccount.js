$(document).ready(function () {
    $('#DefBranches').change(function () {
        ...
        $("#CbAccountsDDLTree").data("kendoDropDownTree").value("");
        $("#CbAccountsDDLTree").data("kendoDropDownTree").dataSource.read();
    ...
    $("#CbAccountsDDLTree").kendoDropDownTree({
    ...
    var dataSource = new kendo.data.TreeListDataSource({
        transport: {
            read: {
                url: "/CbCashAndBankAccount/GetAllMainAccountsWithDescendants",
                dataType: "json",
                //dataType: "jsonp"
                Type: "GET"
            },
            parameterMap: function (data, action) {
                ...
                if (action === "read") {
                    return {
                        ...
                        ids: $("#CbAccountsDDLTree").data("kendoDropDownTree").value().join(", "),
                        date: $("#Date").val(),
                        isActive: isActive,
    ...
    var grid = $("#CbCashBankAccountList").kendoTreeList({
        ...
        //toolbar: ["excel"],
        excel: {
            fileName: "Cb Cash Bank Accounts.xlsx",
        ...
        columns: [
            ...
            { width: Resources.DoubleActionWidth, template: "#if(isMainAccount != true){ #<a  href='/CbCashAndBankAccount/Edit/#= id #'  class='btn btn-success btn-sm btnEdit'><i class='fas fa-edit'></i></a> <a  class='btn btn-danger btn-sm btnDelete' ><i class='fas fa-trash-alt'></i></a> #}#" }
        ],
    ...
    $('#CbCashBankAccountList').data('kendoTreeList').dataSource.page(1); // select current page
    grid.data("kendoTreeList").table.on("click", ".btnDelete", removeAccount);

    function removeAccount() {

        var row = $(this).closest("tr"),
            grid = $("#CbCashBankAccountList").data("kendoTreeList"),
        ...
        swal({
            ...
            setTimeout(function () {
                $.ajax({
                    url: "/CbCashAndBankAccount/Delete?id=" + dataItem.id,
                    type: "Get",
    ...
    $("#btnDataReview").on('click', function () {
        ...
        var accountIds = $("#CbAccountsDDLTree").data("kendoDropDownTree").value().join(", ");
        var date = $("#Date").val();
    ...
    function RefreshGrid() {
        $('#CbCashBankAccountList').data('kendoTreeList').dataSource.read();
    }
...
$(".exportExcel").on('click', function () {
    $("#CbCashBankAccountList").getKendoTreeList().saveAsExcel();
});
