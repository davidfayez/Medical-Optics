$(document).ready(function () {
    ...
    $('#DefBranches').change(function () {
        ...
        loadAccountComboTree(false);
    });

    // Como Tree
    var branchId = parseInt($("#FK_DefBranchId").val());

    var accountComboTree;
    loadAccountComboTree(true);
    function loadAccountComboTree(firstRequest) {
        branchId = parseInt($("#FK_DefBranchId").val());

        $.ajax({
            url: "/GlAccount/GetAllAccountsForComboTree",
            type: "Get",
            data: { defBranchId: branchId },
            dataType: "json",
            cache: false,
            success: function (data) {

                if (firstRequest) {
                    accountComboTree = $('#accountComboTree').comboTree({
                        source: data,
                        isMultiple: true,
                        cascadeSelect: true,
                        collapse: true,
                    ...
                    accountComboTree.setSource(data);
                }
            },
            error: function (data) {
        ...
        transport: {
            ...
            parameterMap: function (data, action) {
                ...
                if (action === "read") {
                    ...
                    if (accountComboTree != undefined)
                        SelectedIds = accountComboTree.getSelectedIds();
                    if (SelectedIds != null)
                        ids = SelectedIds.join(", ");
                    //var ids = accountComboTree.getSelectedItemsId();
        ...
        columns: [
            ...
            },
            {
                ...
                columns: [{
                    ...
                    // template: "#: kendo.toString(openingDebit, '0.00')#",
                    format: '{0:n2}',
                    ...
                    width: Resources.AmountWidth,
                    format: '{0:n2}',
                    // template: "#: kendo.toString(openingCredit, '0.00')#",
                    footerTemplate: Resources.Total + ": #: kendo.toString(sum, '0.00') # "
                }]
            },
            {
                ...
                columns: [{
                    field: "previousTransactionDebit",
                    title: Resources.DebitResource,
                    width: Resources.AmountWidth,
                    format: '{0:n2}',
                    ...
                    field: "previousTransactionCredit",
                    title: Resources.CreditResource,
                    width: Resources.AmountWidth,
                    format: '{0:n2}',
            ...
            //},
            {
                ...
                columns: [{
                    ...
                    width: Resources.AmountWidth,
                    format: '{0:n2}',
                    // template: "#: kendo.toString(currentDebit, '0.00')#",
                    footerTemplate: Resources.Total + ": #: kendo.toString(sum, '0.00') # ",
                }, {
                    field: "currentCredit",
                    title: Resources.CreditResource,
                    width: Resources.AmountWidth,
                    format: '{0:n2}',
                    // template: "#: kendo.toString(currentCredit, '0.00')#",
                    footerTemplate: Resources.Total + ": #: kendo.toString(sum, '0.00') # ",
                }]
            },
    ...
    $("#btnDataReview").click(function () {
        ...
        var names = accountComboTree.getSelectedNames();
        var ids = "";
        var SelectedIds = accountComboTree.getSelectedIds();
        if (SelectedIds != null)
            ids = SelectedIds.join(", ");
    ...
    $(".btnPrint").on('click', function () {
        ...
        var costCenterId = parseInt($("#FK_CostCenterId").val()),
            isTreeDetails = $("#IsTreeDetails").prop("checked"),
            isMainAccounts = $("#IsMainAccounts").prop("checked"),
        ...
        var url = "/GlReports/MovementBalanceAndAuditReportPrint?ids=" + ids + "&costCenterIds=" + costCenterIds + "&dateFrom=" + dateFrom + "&dateTo=" + dateTo + "&isTotalAccountZero=" + isTotalAccountZero + "&isSubAccountSupport=" + isSubAccountSupport + "&fK_DefBranchId=" + fK_DefBranchId + "&taxIds=" + taxIds + "&empIds=" + empIds + "&isTreeDetails=" + isTreeDetails + "&isMainAccounts=" + isMainAccounts;
...
function SetSummary() {
    ...
    if (treeData != null && treeData != undefined) {
        ...
        if (lastRowIdx >= 0) {
            ...
            sumTotalCredit = treeData[lastRowIdx].sumPreviousTransactionCredit;
            sumTotalDebit = treeData[lastRowIdx].sumPreviousTransactionDebit;
    ...
    sumTotalCredit = numberWithCommas(parseFloat(sumTotalCredit).toFixed(2));
    sumTotalDebit = numberWithCommas(parseFloat(sumTotalDebit).toFixed(2));
    ...
    $('#sumTotalCredit').val(sumTotalCredit);
    $('#sumTotalDebit').val(sumTotalDebit);
    $('#sumCredit').val(sumCreditTransaction);
    $('#sumDebit').val(sumDebitTransaction);
